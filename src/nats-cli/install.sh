...
install_deb_using_github() {
    ...
    find_version_from_git_tags CLI_VERSION https://github.com/nats-io/natscli
    cli_filename="nats-${CLI_VERSION}-${arch}.deb"

    mkdir -p /tmp/natscli
    pushd /tmp/natscli
    wget https://github.com/nats-io/natscli/releases/download/v${CLI_VERSION}/${cli_filename}
    exit_code=$?
    set -e
    if [ "$exit_code" != "0" ]; then
        # Handle situation where git tags are ahead of what was is available to actually download
        echo "(!) nats-cli version ${CLI_VERSION} failed to download. Attempting to fall back one version to retry..."
        find_prev_version_from_git_tags CLI_VERSION https://github.com/nats-io/natscli
        wget https://github.com/nats-io/natscli/releases/download/v${CLI_VERSION}/${cli_filename}
    fi

    dpkg -i /tmp/natscli/${cli_filename}
    popd
    rm -rf /tmp/natscli
}
