...
ADDITIONAL_VERSIONS=${ADDITIONALVERSIONS:-"$DOTNET_LTS,$DOTNET_LATEST"}
...
if [ ! -z "${ADDITIONAL_VERSIONS}" ]; then
    ...
    IFS=","
        ...
        for version in "${additional_versions[@]}"; do
            ...
            install_using_apt "${DOTNET_SDK_OR_RUNTIME}" "${version}" "true"
        done
    IFS=$OLDIFS
fi
...
updaterc "export PATH=\"/home/$USERNAME/.dotnet/tools:\$PATH\""
